...
public partial class matrix
{
    ...
    public static bool approx(matrix A, matrix B, double acc = 1e-9, double eps = 1e-9)
    {
        ...
        for (int i = 0; i < A.size1; i++)
            for (int j = 0; j < A.size2; j++)
                if (!vector.approx(A[i, j], B[i, j], acc, eps)) return false;
        return true;
    }
    ...
    public void print(string s = "", string format = "{0,10:g3} ", TextWriter file = null, double threshold = 1e-9)
    {
        ...
        for (int ir = 0; ir < this.size1; ir++)
        {
            ...
            {
                double value = this[ir, ic];

                // Check for negative zero and replace with zero
                if (Math.Abs(value) < threshold || value == 0) value = 0;
